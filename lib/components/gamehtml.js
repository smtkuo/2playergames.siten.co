...
module.exports = function (data, query) {
	...
	if(data['hideMobileTitle'] == null){
		...
		<div class="m-2 overflow-auto block md:hidden">
			<div class="alert backdrop-blur-sm backdrop-brightness-125 bg-transparent shadow-lg shadow-lg">
			<div>
	...
	if (data['gamelist'][0] != null) {
		...
		for (var key in data['gamelist'][0]) {
			try{
				...
				// IMAGES END
				object['gameHtml'] += `
					<div class="w-5/12 md:w-64 card  backdrop-blur-sm backdrop-brightness-125 bg-transparent m-2 cursor-pointer" onclick='window.location.href = "${process.env.PROJECT_Main}/play-game/${value['id']}-${Object.index.convertToSlug(value["Title"])}";'>
	...
	<div class="p-2 py-6"> <div class="alert backdrop-blur-sm backdrop-brightness-125 bg-transparent shadow-lg shadow-lg"> <div> <span class="card-title text-primary-content"><i class="fa fa-bullhorn m-1" aria-hidden="true"></i> Share</span> </div> <div class="addthis_inline_share_toolbox_txyg"></div> </div></div>
	`;
	object['randomGamesHtml'] += '<div class="p-2 py-6"> <div class="alert backdrop-blur-sm backdrop-brightness-125 bg-transparent shadow-lg shadow-lg"> <div> <span class="card-title text-primary-content"><i class="fa fa-random m-1" aria-hidden="true"></i> Random Games</span> </div> </div></div>'; 
